...
class AzureDevopsProvider(GitProvider):
    ...
    def publish_code_suggestions(self, code_suggestions: list) -> bool:
        ...
        for suggestion in code_suggestions:
            ...
            comment = Comment(content=AzureDevopsProvider._add_footer(body), comment_type=1)
    ...
    def edit_comment(self, comment: Comment, body: str):
        try:
            self.azure_devops_client.update_comment(
                ...
                comment=Comment(content=AzureDevopsProvider._add_footer(body)),
                project=self.workspace_slug,
            )
        except Exception as e:
    ...
    def publish_comment(self, pr_comment: str, is_temporary: bool = False, thread_context=None) -> Comment:
        if is_temporary and not get_settings().config.publish_output_progress:
            ...
            return None
        comment = Comment(content=AzureDevopsProvider._add_footer(pr_comment))
    ...
    def publish_description(self, pr_title: str, pr_body: str):
        pr_body = AzureDevopsProvider._add_footer(pr_body)
    ...
    def _add_footer(body: str) -> str:
        if body and body[-1] != "\n":
            body += "\n"
        body += "\n___\n"
        body += "> This comment was generated by AI using [pr-agent](https://github.com/qodo-ai/pr-agent)"
        return body
    
    def description_is_generated_by_pr_agent(self) -> bool:
        description = (self.get_pr_description_full() or "").strip()
        description_lowercase = description.lower()
        return self._is_generated_by_pr_agent(description_lowercase)
